...
public class PushPayloadService implements BeanPropertiesUtil{
    ...
    public PushResult sendPushWithRegIds(PushDTO pushDTO) throws IOException {
        ...
        List<String> regIds = resultData.stream().filter(usersVO -> usersVO.getDeviceId() != null).filter(usersVO -> usersVO.getLoggedIn() == 1).map(usersVO -> usersVO.getDeviceId()).collect(Collectors.toList());
